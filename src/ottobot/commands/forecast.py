...
def summarize_forecast_text(text: str) -> str:
    ...
    clean = re.sub(r'(\d+)\s*percent chance of showers changing to (\d+)\s*percent chance of showers', r'\1->\2% shwrs', clean, flags=re.IGNORECASE)
    clean = re.sub(r'(\d+)\s*percent chance of showers', r'\1% shwrs', clean, flags=re.IGNORECASE)
    ...
    # Fix orphan spaces before commas caused by word removal
    clean = re.sub(r'\s+,', ',', clean)
...
def fetch_forecast_datamart() -> str:
    ...
    req = urllib.request.Request(
        URL, 
        headers={'User-Agent': 'Mozilla/5.0 (Ottobot Mesh Node)'}
    )
